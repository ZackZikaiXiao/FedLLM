...
class DataTokenizer:
    ...
    def generate_and_tokenize_prompt(self, data_point):
        return self._generate_and_tokenize(data_point)
    ...
    def _generate_and_tokenize(self, data_point):
        ...
        # tokenization
        tokenized_full_prompt = self._tokenize_(full_prompt)   # {input_ids; attention_mask; labels}
        if not self.args.train_on_inputs:
            user_prompt = self.prompter.generate_prompt(
                data_point["instruction"], data_point["context"]
            )
            tokenized_user_prompt = self._tokenize_(user_prompt, add_eos_token=False)
            user_prompt_len = len(tokenized_user_prompt["input_ids"])
            tokenized_full_prompt["labels"] = [-100] * user_prompt_len + tokenized_full_prompt["labels"][user_prompt_len:]  # could be sped up, probably
        return tokenized_full_prompt
    
    
    def _tokenize_(self, prompt, add_eos_token=True):
        result = self.tokenizer(
            text=prompt,
            truncation=True,
            max_length=self.args.cutoff_len,
